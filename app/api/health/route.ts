...
export function GET(): Response {
  const hasMcpEndpoint = Boolean(
    process.env.BRAINWORM_MCP_URL?.startsWith("https://") &&
      process.env.BRAINWORM_MCP_LABEL,
  );
  const hasBuildPolicy = Boolean(
    process.env.BRAINWORM_MCP_ALLOWED_TOOLS ||
      process.env.BRAINWORM_MCP_ALLOW_ALL === "true",
  );
  return Response.json(
    {
      ...
      mcpReadOnlyConfigured: Boolean(
        hasMcpEndpoint && process.env.BRAINWORM_MCP_READONLY_TOOLS,
      ),
    },
    {
      headers: {
