...
function message(
  ...
  return { id, role, content, createdAt: 0, status };
}
...
describe("estimateActiveHistoryTokens", () => {
  ...
  it("counts the summary plus the uncompacted tail", () => {
    ...
    const withoutSummary = estimateActiveHistoryTokens([messages[1]], undefined, undefined);
    const combined = estimateActiveHistoryTokens(messages, "abcd", "1");
    expect(combined).toBe(1 + withoutSummary);
  });

  it("ignores empty, error, and streaming messages in the tail", () => {
    const messages = [
      message("1", "user", "   "),
      message("2", "assistant", "real content", "error"),
      message("3", "assistant", "still streaming", "streaming"),
    ];
...
describe("buildCompactionRequestContent", () => {
  ...
  it("omits empty and incomplete messages from the transcript", () => {
    const tail = [
      message("1", "user", "   "),
      message("2", "assistant", "broken", "error"),
      message("3", "user", "kept"),
    ];
    const content = buildCompactionRequestContent(undefined, tail);
