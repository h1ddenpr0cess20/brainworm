...
// oldest turns fall out the server's MAX_MESSAGES/MAX_TOTAL_CHARACTERS caps
// (app/api/chat/route.ts), compaction folds them into a running summary that
// travels in the system prompt instead. Each compaction combines whatever
// summary already exists with every message since the last compaction, so
// the model asked to write the new summary sees the full conversation to
// date rather than just the latest slice — otherwise repeated compactions
// would each forget what the previous one had already condensed.

import { estimateMessageTokens, estimateTokens } from "./tokenBudget";
import type { Message } from "./types";

function isFoldable(message: Message): boolean {
  return Boolean(message.content.trim()) && message.status === "complete" && !message.notice;
}
...
export function uncompactedMessages(
  ...
  const index = messages.findIndex((message) => message.id === compactedThroughId);
  return index === -1 ? messages : messages.slice(index + 1);
}

/** Estimated tokens still in play for a conversation: summary plus uncompacted tail. */
export function estimateActiveHistoryTokens(
  messages: Message[],
  compactedSummary: string | undefined,
  compactedThroughId: string | undefined,
): number {
  const tail = uncompactedMessages(messages, compactedThroughId).filter(isFoldable);
  const tailTokens = tail.reduce((sum, message) => sum + estimateMessageTokens(message), 0);
